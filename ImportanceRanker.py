...
class Train_CV_SGBRT(object):
    ...
    def __init__(self):
        self.data_path = sparkTunerWarehouse + "/SparkTunerReports"
        ...
        for _ in range(Itera):
            ...
            if (importances_sum < 0.9999) or (importances_sum > 1.0002):
                for i in range(len(Events_Name[_ - 1])):
                    importanceRanks = []
                    featureName = envParser.sparkParametersWithSql[
                        int("".join(list(filter(str.isdigit, Events_Name[_ - 1][i])))) - 1]
                    importanceRanks.append(featureName)
                    importanceRanks.append(Importances[_ - 1][i])
                    importanceRanksCsv.append(importanceRanks)
                with open(str(sparkTunerWarehouse) + "/SparkTunerReports/outputConfigurationFiles/importanceRanks.csv",
                          "w") as importanceRanksFile:
